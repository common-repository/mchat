...
HTML;
	...
	*/
		...
		if( ! empty( $user_id ) && $user_id == get_current_user_id() ) {
			return json_encode( array( 'err'=> sprintf( __('Error %d: Cant Chat with your self','wpmchat_lang' ), __LINE__ ) ) );
		}
	...
	**/
	...
	function wpmchat_user_can( $user_id )
	{
		...
		$ret = [];
		if( ! empty( $roles ) && is_array( $roles ) && ! empty( $wpmchat_settings['role_list'] ) && is_array( $wpmchat_settings['role_list'] ) ) {
	...
	**/
	...
	function wpmchat_heartbeat( $chat_id, $user_id, $inbox_updated )
	{
		...
		if( ! empty( $last_message ) )
			$last_message = strtotime( $last_message );
		else
			$last_message = current_time('timestamp');

		//get inbox - whole of it//
		if( $last_message > $inbox_updated ) {
		...
		if( $chat_id > 0 && $last_message > $inbox_updated ){
	...
	function wpmchat_ajax_chat_action()
	{
		...
		if( ! $this->wpmchat_user_can( get_current_user_id() ) ) {
			$out = array();
			$out['msg'] = __('You are not authorized to use this chat.','wpmchat_lang');
			$out['err'] = __LINE__;
			header( "Content-Type: application/json" );
			echo json_encode( $out );
			die();
	...
	function log( $function, $line, $str )
	{
		$str = "\n[".current_time('mysql')."] ".$function.":".$line.": ".$str;
